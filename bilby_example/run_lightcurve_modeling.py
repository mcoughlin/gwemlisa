...
with open(binarydir.joinpath(f'{binary.name}_parameters.json'), 'w+') as parameters:
    json.dump(parameter_dict(b), parameters, indent=2)
...
fdot_log10 = [np.log10(x) for x in fdotgw(mchirp, b.f0)]

# Produce corner plot of chirp mass against log10(fdot)
...
def k2_model(p0, mchirp, incl, q):
    return 2*np.pi*sma(p0, mchirp, q)*np.sin(np.radians(incl))/((1+q)*(p0*(60*60*24)))

# Set up likelihood function
std_k2 = 50
likelihood = GaussianLikelihood(b.p0, b.k2, k2_model, std_k2)
